...
function buildRealKCCEpub() {
    console.log("Generating with Archiver using REAL image...");
    const baseDir = path.join(__dirname, 'advanced_8_dir');
...
display: none;
    ...
    const realImgPath = path.join(__dirname, 'real_comic_page.jpg');
    fs.copyFileSync(realImgPath, path.join(baseDir, 'OEBPS', 'Images', 'kcc-0000-kcc.jpg'));
    fs.copyFileSync(realImgPath, path.join(baseDir, 'OEBPS', 'Images', 'cover.jpg'));
    
    const dims = getJpegSize(realImgPath);
...
<dc:title>Hunter and Hunted</dc:title>
...
<div id="PV">
    ...
    const outputPath = path.join(__dirname, 'advanced_8_wolverine_mimic.epub');
