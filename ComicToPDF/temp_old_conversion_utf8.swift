...
class ConversionManager: ObservableObject {
    ...
    // Γ£à NEW: Persistent Watched Folders
    ...
    // Γ£à NEW: Precision Canvas Models (Normalized Coordinates)
    @Published var pageModels: [UUID: [Int: PageModel]] = [:]  
    
    // Γ£à Helper to get or create PageModel
    ...
    // Γ£à Secure Processing Core Integration
    ...
    func saveLibrary() {
        struct LibraryIndex: Codable {
            ...
            var panelOverrides: [UUID: [Int: [PanelExtractor.Panel]]]? = nil // Γ£à NEW: Persistence
            var watchedFolders: [WatchedFolder]? = nil // Γ£à NEW: Watched Folders
        }
    ...
    func loadLibrary() {
        struct LibraryIndex: Codable {
            ...
            let history: [ConvertedPDF]
            let devices: [KindleDevice]
            var panelOverrides: [UUID: [Int: [PanelExtractor.Panel]]]? = nil // Γ£à NEW
            var watchedFolders: [WatchedFolder]? = nil // Γ£à NEW
        }
        ...
        self.panelOverrides = index.panelOverrides ?? [:] // Γ£à Restore overrides
        self.watchedFolders = index.watchedFolders ?? [] // Γ£à Restore Watched Folders
    }
    ...
    // Γ£à NEW: Centralized manifest logic to fix panel loss
    ...
    func processImportedFiles(urls: [URL]) async {
        for url in urls {
            ...
            // Γ£à NEW: Handle PDF imports differently
            let ext = url.pathExtension.lowercased()
            let documentsDir = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0]

            if ext == "pdf" {
                Task {
                    // Γ£à Offload to ConversionEngine
                    do {
    ...
    func mergePDFs(_ pdfs: [ConvertedPDF], outputName: String, mangaMode: Bool) async {
        ...
        do {
            ...
            isConverting = false; statusMessage = "Γ£à Merge Complete!"; scanLibrary()
    ...
    func convertComic(_ pdf: ConvertedPDF, mangaMode: Bool) async {
        ...
        do {
            if jobSettings.outputFormat == .pdf {
                ...
                isConverting = false; conversionProgress = 1.0; statusMessage = "Γ£à Conversion Complete!"; scanLibrary()
                ...
                isConverting = false; conversionProgress = 1.0; statusMessage = "Γ£à Conversion Complete! (\(newURLs.count) files)"; scanLibrary()
    ...
    func convertAndMerge(sourceFiles: [ConvertedPDF], outputName: String, mangaMode: Bool) async {
        ...
        for (index, file) in sourceFiles.enumerated() {
            ...
            let converter = CBZToEPUBConverter()
            var jobSettings = conversionSettings
            // Γ£à Override Manga Mode for this batch
            jobSettings.mangaMode = mangaMode
            // Γ£à E013 FIX: Conditional Injection
            jobSettings.isGuidedView = jobSettings.enablePanelSplit
            ...
            // Γ£à Log Settings for Debugging (User Request)
            ...
            do {
                ...
                // Γ£à NEW: Inject Metadata into Generated EPUBs
        ...
        do {
            ...
            await MainActor.run {
                self.scanLibrary()
                self.statusMessage = "Γ£à Merge Complete!"
                self.processingStatus = ""
    ...
    nonisolated static func extractCoverImageStatic(from url: URL) -> UIImage? {
        ...
        if ["cbz", "cbr", "zip", "epub"].contains(ext) {
            // Γ£à Security Scope Safety (Paranoid Check)
            let accessing = url.startAccessingSecurityScopedResource()
            defer { if accessing { url.stopAccessingSecurityScopedResource() } }
            ...
            do {
                ...
                // Γ£à Fix: Use localized sort to match Finder/ZipUtilities (1, 2, 10 vs 1, 10, 2)
                ...
                if ext == "epub" {
                    if let mimetypeEntry = archive["mimetype"] {
                        ...
                        if mimetypeEntry.uncompressedSize == 20 {
                            Logger.shared.log("[Flight Recorder] Γ£à Mimetype size is correct (20 bytes)", category: "Debug")
                        } else {
                            Logger.shared.log("[Flight Recorder] Γ¥î Mimetype size is WRONG: \(mimetypeEntry.uncompressedSize)", category: "Debug")
                        }
                        
                        var data = Data()
                        _ = try? archive.extract(mimetypeEntry, consumer: { data.append($0) })
                        if let content = String(data: data, encoding: .ascii) {
                           Logger.shared.log("[Flight Recorder] ≡ƒôä Mimetype Content: '\(content)'", category: "Debug")
                           if content != "application/epub+zip" {
                               Logger.shared.log("[Flight Recorder] Γ¥î Mimetype Content INVALID", category: "Debug")
                           }
                        }
                    } else {
                         Logger.shared.log("[Flight Recorder] Γ¥î Mimetype file MISSING!", category: "Debug")
                    }
                }
    ...
    // Γ£à NEW: Extract Smart Panels from ComicInfo.xml
    func extractSmartPanels(from url: URL) async throws -> [Int: [PanelExtractor.Panel]]? {
        ...
        // Γ£à REPAIR: Check for Denormalized (Pixel) Coordinates
    ...
    func reorderPages(_ pdf: ConvertedPDF, newOrder: [Int]) async throws -> URL {
        ...
        // Γ£à NEW: Prepare Panel Transfer
        ...
        // Γ£à NEW: Update overrides and inject metdata
        await MainActor.run {
    ...
    func extractPages(from pdf: ConvertedPDF, pageIndices: [Int], asImages: Bool) async throws -> URL {
        ...
        // Γ£à NEW: Apply Panel Overrides to the newly created file (In Memory)
        ...
        // Γ£à NEW: Inject Metadata into the File (On Disk)
    ...
    func exportForCloudSync(_ pdf: ConvertedPDF) async -> URL? {
        ...
        if conversionSettings.outputFormat == .pdf {
            ...
            do {
                ...
                Logger.shared.log("Γ¥î PDF Export Failed: \(error)", category: "Export")
                return nil
            }
        }
        ...
        do {
            ...
            Logger.shared.log("Γ¥î Cloud Export Failed: \(error)", category: "Export")
            return nil
        }
    }
    ...
    // Γ£à NEW: Embed currently saved panels into the source file (EPUB/CBZ)
    // Γ£à NEW: Embed currently saved panels into the source file (EPUB/CBZ)
    ...
    // Γ£à NEW: Reusable Metadata Injection with Strict Re-Zip
                    ...
                    if conversionSettings.isGuidedView {
                        if !opfString.contains("rendition:layout") {
                             if let range = opfString.range(of: "</metadata>") {
                                 ...
                                 // Γ£à RESOLUTION EXTRACTION: Find first image to set correct original-resolution
        ...
        do {
            ...
            if let oldArchive = try? Archive(url: archiveURL, accessMode: .read) {
                ...
                for entry in oldArchive {
                    ...
                    if entry.path == "META-INF/ComicInfo.xml" { continue } // Γ£à Skip new location too
...
class ComicInfoPanelParser: NSObject, XMLParserDelegate {
    ...
    private var currentImageSize: CGSize? // Γ£à Support for normalization
        ...
        if cleanName.caseInsensitiveCompare("Page") == .orderedSame {
            ...
            if let xVal = getAttr("x"), let x = Double(xVal),
               ...
               let hVal = getAttr("height"), let h = Double(hVal) {
                ...
                // Γ£à Auto-Normalize if values are in Pixels
