...
final class HighlightExportService {
    ...
    /// Generates Markdown formatted highlights document from AnnotationStore annotations
    func exportToMarkdown(bookTitle: String, author: String?, storeAnnotations: [Annotation]) -> String {
        var md = "# Highlights from \(bookTitle)\n"
        if let author = author, !author.isEmpty {
            md += "*by \(author)*\n\n"
        } else {
        ...
        let sorted = storeAnnotations.sorted {
            if $0.pageIndex != $1.pageIndex {
                return $0.pageIndex < $1.pageIndex
            }
            return $0.createdAt < $1.createdAt
    ...
    /// Generates Readwise-compatible CSV formatted highlights from AnnotationStore annotations
    func exportToReadwiseCSV(bookTitle: String, author: String?, storeAnnotations: [Annotation]) -> String {
        var csv = "Highlight,Book Title,Book Author,Location,Date,Note\n"
