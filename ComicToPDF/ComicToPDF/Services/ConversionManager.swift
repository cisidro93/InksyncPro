...
class ConversionManager: ObservableObject {
    ...
    // MARK: - Safe Extraction (Simple & Robust)
    // This is the crash-proof version. It only unzips. No image loading.
    ...
    nonisolated static func extractCoverImageStatic(from url: URL) -> UIImage? {
        ...
        if ["cbz", "cbr", "zip", "epub"].contains(ext) {
            // Updated to handle both deprecated and throwing init if needed, using safe try?
    ...
    // MARK: - Page Operations (Delete/Reorder)
